      ...
      {!isLoading ? (
        <div>
          <div className="semi-bolder  text-2xl font-semibold leading-loose text-foreground">
            {isValidatorWallet ? (
              <div className="flex cursor-pointer items-center" onClick={handleOpenValidatorPortal}>
          ...
          <div className="mt-2 flex items-center gap-2 text-sm font-semibold text-muted-foreground">
            {isValidatorWallet && <Icons.checkCircle className="h-6 w-6 text-positive" />}
            {isConnected && !isWrongNetwork && <div>Connected as </div>}
            <ConnectButton isPopover={false} className="max-w-[300px]" />
          </div>
        </div>
      ) : (
