	...
	[507],
	{
		79797: (t, e, i) => {
			...
			class p extends d.MediaCoordinatesPaneRenderer {
				...
				_drawImpl(t) {
					...
					const d = null !== (e = this._data.linecap) && void 0 !== e ? e : o;
					(s.lineCap = d),
		...
		47372: (t, e, i) => {
			...
			class o {
				...
				drawBackground(t, e) {
					const i = this._data;
					const s = (0, a.generateColor)(
						this._data.color,
						this._data.transparency,
					);
					const r = e.pixelRatio;
					const o = Math.round(i.x1 * r);
					const l = Math.round(i.x2 * r);
		...
		21335: (t, e, i) => {
				...
				hitTest(t, e) {
					...
					for (const s of i.histograms) {
						...
						for (const o of s.bars) {
							...
							const { xBasePoint: h, sign: c, xRangeWidth: u } = d;
							const p = Math.max(
							...
							for (let e = 0; e < o.subBarValues.length; e++) {
								...
								const a = _;
								const l = a + c * ((p * o.subBarValues[e]) / r);
								if (
									((_ = l),
					...
					t.save(),
						i.histograms.forEach((n) => {
							if (i.histBoxBgColor) {
								...
								const s = Math.round(n.yRange.min * r);
								const a = Math.round(n.xRange.max * r) - e;
								const o = Math.round(n.yRange.max * r) - s;
							...
							if (
								(n.bars.forEach((s) => {
									...
									const { xBasePoint: u, sign: p, xRangeWidth: _ } = d;
									if (s.y > e.cssHeight || s.y + s.height < 0) return;
									const m = Math.max(
										(l.percentWidth * _) / 100 - s.subBarValues.length,
										0,
									);
									let x = u;
									for (let e = 0; e < s.subBarValues.length; e++) {
										const i = s.y;
										const n = s.y + s.height;
										const a = x;
										const d = a + p * ((m * s.subBarValues[e]) / o);
										if (Math.abs(d - a) < 0.5) continue;
										(x = d), t.beginPath(), (t.fillStyle = l.colors[e]);
										const c = Math.round(a * r);
										const u = Math.round(i * r);
										const _ = Math.round(d * r) - c;
										const f = Math.max(Math.round(n * r) - u - h, 1);
										t.rect(c, u, _, f), t.fill();
									}
									if (!l.showValues) return;
									const y = v(s.displayedValues, l.direction);
									const w = g(_, s.y, s.height, d, l, y);
									c.push(w);
								}),
								a.length > 0)
							) {
								const t = i.styles[n.bars[0].styleId];
								const r = f(n.xRange, t, e.cssWidth);
								const o = v(a, t.direction);
								const l = n.bars[n.bars.length - 1];
								const h = g(r.xRangeWidth, l.y + l.height, d, r, t, o);
			...
			function f(t, e, i) {
				...
				const r = e.location === a.HHistLocation.Relative;
				const n = e.direction === a.HHistDirection.LeftToRight;
				const o = e.direction === a.HHistDirection.RightToLeft;
				let l;
				let d;
				if (s && n) (l = t.min), (d = 1);
			...
			function g(t, e, i, s, r, n) {
				...
				const l =
					r.direction === a.HHistDirection.LeftToRight ? "left" : "right";
				const { xBasePoint: h, sign: c } = s;
				const u = h + 3 * c;
				const p = e + 0.7 * i;
				return {
					text: n,
			...
			class y {
				...
				_updateDataForRenderers(t, e, i) {
					...
					const c = (0, r.ensureNotNull)(this._provider.firstValue());
					const u = [];
					t.forEach((t) => {
		...
		13369: (t, e, i) => {
			...
			class o {
				...
				_updateViewInternal() {
					...
					const a = r.firstBar();
					const o = r.lastBar();
					const l = this._extendToBarsEndings ? e.barSpacing() / 2 : 0;
					const d = (0, s.ensureDefined)(
		...
		66999: (t, e, i) => {
			...
			class d {
				...
				_updateViewInternal() {
					...
					const l = n.firstBar();
					const d = n.lastBar();
					const h = (0, s.ensureDefined)(
						this._provider.properties().childs().graphics.childs().polygons,
					);
					const c = (0, s.ensureDefined)(
						this._provider.graphicsInfo().polygons,
					);
					i.forEach((i, n) => {
						const u = (0, s.ensureDefined)(h.childs()[n]).childs();
						const p = (0, s.ensureDefined)(c[n]);
		...
		78266: (t, e, i) => {
			...
			class l {
				...
				_updateViewInternal() {
					...
					const r = this._provider.graphics().vertlines();
