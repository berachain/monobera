		...
		63016: (t, e, i) => {
			...
			class d extends s.PureComponent {
				...
				render() {
					...
					const p = (0, l.filterDataProps)(this.props);
					const g = this.props.style
			...
			class v {
				...
				_moveDialog(t, e) {
					...
					const { boundByScreen: s } = this._options;
					const o = g(
						t,
						i.width,
						s ? 0 : -1 / 0,
						s ? window.innerWidth : 1 / 0,
					);
					const n = g(
						e,
						i.height,
						s ? 0 : -1 / 0,
						s ? window.innerHeight : 1 / 0,
					);
			...
			class D {
				...
				centerAndFit() {
					...
					const i = this._calcAvailableHeight();
					const s = this._calcDialogHeight();
					if (i === s)
				...
				getDialogsTopLeftCoordinates() {
					...
					const i = this._calcDialogHeight();
					const s = e / 2 - this._dialog.clientWidth / 2;
					const o = t / 2 - i / 2;
				...
				recalculateBounds() {
					...
					if (this._isFullscreen) {
						...
						const o = e - 2 * s;
						const n = g(t.left, t.width, 0, i);
						const a = g(t.top, t.height, s, e);
						(this._dialog.style.top = "0px"),
							(this._dialog.style.left = "0px"),
							(this._dialog.style.transform = `translate(${Math.round(
								n,
			...
			class M extends s.PureComponent {
				...
				componentDidMount() {
					...
					const s = (0, n.ensureNotNull)(this._dialog);
					if (t) {
				...
				_applyAppearanceAnimation(t) {
					if (this._resize && this._dialog) {
						const { x: e, y: i } = t;
						const { x: s, y: o } = this._resize.getDialogsTopLeftCoordinates();
