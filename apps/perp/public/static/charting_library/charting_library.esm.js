...
const s = class {
	constructor(n) {
		...
		(null === (o = n.overrides) || void 0 === o
			...
			(n.overrides["mainSeriesProperties.priceAxisProperties.lockScale"] =
				undefined)),
			(this._options = t(e, n));
		"dark" ===
	...
	_create() {
		...
		const n =
			...
			void 0 !== e &&
			e;
		const [i, o] = this._render(!n);
		const s = this._options.container;
		const r = "string" === typeof s ? document.getElementById(s) : s;
		if (null === r)
	...
	_render(t) {
		...
		const s = `symbol=${encodeURIComponent(
			this._options.symbol || "",
		)}&interval=${encodeURIComponent(this._options.interval)}${
			this._options.toolbar_bg
				? `&toolbarbg=${encodeURIComponent(
						this._options.toolbar_bg.replace("#", ""),
				  )}`
				: ""
		}${
			this._options.studies_access
				? `&studiesAccess=${encodeURIComponent(
						JSON.stringify(this._options.studies_access),
				  )}`
				: ""
		}&widgetbar=${encodeURIComponent(JSON.stringify(this._options.widgetbar))}${
			this._options.drawings_access
				? `&drawingsAccess=${encodeURIComponent(
			...
			this._options.locale,
		)}&uid=${encodeURIComponent(this._id)}&clientId=${encodeURIComponent(
			String(this._options.client_id),
		)}&userId=${encodeURIComponent(String(this._options.user_id))}${
			this._options.charts_storage_url
				? `&chartsStorageUrl=${encodeURIComponent(
						this._options.charts_storage_url,
				  )}`
				: ""
		}${
			this._options.charts_storage_api_version
				? `&chartsStorageVer=${encodeURIComponent(
						this._options.charts_storage_api_version,
				  )}`
				: ""
		}${
			...
			this._options.custom_font_family
				? `&customFontFamily=${encodeURIComponent(
						this._options.custom_font_family,
				  )}`
				: ""
		}${
			this._options.auto_save_delay
				? `&autoSaveDelay=${encodeURIComponent(
						String(this._options.auto_save_delay),
				  )}`
				: ""
		}&debug=${encodeURIComponent(String(this._options.debug))}${
			...
			this._options.header_widget_buttons_mode
				? `&header_widget_buttons_mode=${encodeURIComponent(
						String(this._options.header_widget_buttons_mode),
				  )}`
				: ""
		}${
			this._options.time_scale
				? `&time_scale=${encodeURIComponent(
