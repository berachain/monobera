...
export async function GET(
    ...
    const data: any = await Promise.all([swaps, adds, removes]).then(
      ([swaps, adds, removes]) => ({
        swaps: swaps.result,
        adds: adds.result,
    ...
    if(swap === null && provisions === null) {
      sortedData = sortByBlockTime([...data.swaps, ...data.adds, ...data.removes]);
    }
    if(swap !== null && provisions === null) {
    ...
    if(page && !perPage) {
      paginatedData = paginatedData.slice((parseInt(page) - 1) * DEFAULT_SIZE, parseInt(page) * DEFAULT_SIZE);
    }
    if(!page && perPage) {
      paginatedData = paginatedData.slice(0, parseInt(perPage));
    }
    if(page && perPage) {
      paginatedData = paginatedData.slice((parseInt(page) - 1) * parseInt(perPage), parseInt(page) * parseInt(perPage));
    }
