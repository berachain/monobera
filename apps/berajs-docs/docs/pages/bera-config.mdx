...
You can import the default beraJsConfig object
...
:::info
Note that if you're developing in the monorepo itself, `beraJsConfig` is populated at build time with environment variables from `.env` and `.env.local` at the monorepo root with the following mapping.
:::
