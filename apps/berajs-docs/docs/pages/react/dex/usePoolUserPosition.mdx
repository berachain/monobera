...
React SWR hook to fetch the current connected wallet's user stake position in a given pool. Connected wallet is retrieved by using [wagmi's `useAccount` hook](https://wagmi.sh/react/api/hooks/useAccount).
