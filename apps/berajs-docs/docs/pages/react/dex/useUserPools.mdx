...
React SWR hook that, given a user's wallet address and optional search keyword, returns a list of pools that a user has positions including the user's position data per pool.
