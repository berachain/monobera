...
Most actions accept a `config` argument of type `BeraConfig`, this overrides the default configuration of contract and endpoint addresses that the actions act on. [Learn more about the `config` argument here.](/bera-config)
