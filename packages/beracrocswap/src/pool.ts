...
export class CrocPoolView {
    ...
    async mintAmbientBase (qty: TokenQty, limits: PriceRange,shareAddress: string, opts?: CrocLpOpts): 
        Promise<BeraSdkResponse> {
        return this.mintAmbient(qty, this.useTrueBase, limits, shareAddress, opts)
    }

    async mintAmbientQuote (qty: TokenQty, limits: PriceRange,shareAddress: string,opts?: CrocLpOpts): 
        Promise<BeraSdkResponse> {
        return this.mintAmbient(qty, !this.useTrueBase, limits,shareAddress, opts)
    }
    ...
    async burnAmbientLiq (poolIdx: number, liq: BigNumber, limits: PriceRange,shareAddress: string, opts?: CrocLpOpts): 
        Promise<BeraSdkResponse> {
        ...
        const calldata = (await this.makeEncoderPoolIDx(poolIdx)).encodeBurnAmbient
            (liq, lowerBound, upperBound, this.maskSurplusFlag(opts),shareAddress)
        return {
            calldata
        }
    }

    async burnAmbientAll (poolIdx: number, limits: PriceRange,shareAddress: string, opts?: CrocLpOpts): Promise<BeraSdkResponse> {
        let [lowerBound, upperBound] = await this.transformLimits(limits)
        const calldata = (await this.makeEncoderPoolIDx(poolIdx)).encodeBurnAmbientAll
            (lowerBound, upperBound, this.maskSurplusFlag(opts),shareAddress)
        return {
            calldata
        }
    }
