...
export function EVMgetOutputAmountSwap(
  ...
  if (swapType === SwapTypes.SwapExactIn) {
    ...
    if (amount >= poolPairData.balanceOut) return INFINITY;
  }
  if (swapType === SwapTypes.SwapExactIn) {
  ...
  pool.updateTokenBalanceForPool(tokenIn, balanceIn + amountIn);
  pool.updateTokenBalanceForPool(tokenOut, balanceOut - amountOut);
