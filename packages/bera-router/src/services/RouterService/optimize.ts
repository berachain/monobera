...
export const optimizeSwapAmounts = (
  ...
  for (let b = initialNumPaths; b <= paths.length; b++) {
    if (b != initialNumPaths) {
      ...
      swapAmounts.forEach((swapAmount, i) => {
        swapAmounts[i] =
          swapAmount * (1n - newSwapAmount / humanTotalSwapAmount);
      });
    ...
    if (swapType === SwapTypes.SwapExactIn) {
      ...
      improvementCondition =
        totalReturnConsideringFees > bestTotalReturnConsideringFees;
    } else {
      totalReturnConsideringFees = totalReturn + gasFees;
      improvementCondition =
        totalReturnConsideringFees < bestTotalReturnConsideringFees;
  ...
  bestSwapAmounts = bestSwapAmounts.filter((swapAmount) => swapAmount !== 0n);
...
export const formatSwaps = (
  ...
  if (swaps.length > 0 && swaps[0]) {
    ...
    if (swapType === SwapTypes.SwapExactIn) {
      ...
      swaps[0][0].swapAmount = (
        BigInt(swaps[0][0]?.swapAmount ?? "0") + dust
      ).toString();
    } else {
      ...
      swaps[0][firstPathLastPoolIndex].swapAmount = (
        BigInt(swaps[0][firstPathLastPoolIndex]?.swapAmount ?? "0") + dust
      ).toString();
    }
  }
  ...
  const marketSp = getSpotPriceAfterSwapForPath(largestSwapPath, swapType, 0n);
...
function getBestPathIds(
  ...
  sortedSwapAmounts.forEach((swapAmount) => {
    ...
    paths.forEach((path, i) => {
      ...
      if (path.limitAmount >= swapAmount) {
    ...
    if (bestPathIndex === -1) {
      ...
      selectedPathExceedingAmounts.push(
        swapAmount - (paths[bestPathIndex]?.limitAmount ?? 0n),
      );
...
function iterateSwapAmounts(
  ...
  while (priceError > PRICE_ERROR_TOLERANCE) {
    [prices, swapAmounts, exceedingAmounts] = iterateSwapAmountsApproximation(
      selectedPaths,
      swapType,
      totalSwapAmount,
      swapAmounts,
...
function iterateSwapAmountsApproximation(
  ...
  swapAmounts.forEach((swapAmount, i) => {
    if (
      ...
      sumSPaSDividedByDerivativeSPaSs += SPaS / derivative_SPaS;
    } else {
  ...
  const weighted_average_SPaS =
    sumSPaSDividedByDerivativeSPaSs / sumInverseDerivativeSPaSs;
  ...
  swapAmounts.forEach((swapAmount, i) => {
    if (
      ...
      const deltaSwapAmount =
        weighted_average_SPaS - (SPaSs[i] ?? 0n) / (derivativeSPaSs[i] ?? 0n);
  ...
  while (whileLoopCondition(swapAmounts, exceedingAmounts)) {
  ...
  swapAmounts.forEach((swapAmount, i) => {
    ...
    if (
      ...
      pricesForViableAmounts.push(
        getSpotPriceAfterSwapForPath(selectedPaths[i], swapType, swapAmount),
      );
    }
  });
...
export const calcTotalReturn = (
  ...
  paths.forEach((path, i) => {
    totalReturn += getOutputAmountSwapForPath(
      path,
      swapType,
      swapAmounts[i] ?? 0n,
      inputDecimals,
...
export function getEffectivePriceSwapForPath(
  ...
  const gasCost =
    parseUnits(`${Number(costReturnToken)}`, outputDecimals) *
    BigInt(path.pools.length);
  if (swapType === SwapTypes.SwapExactIn) {
