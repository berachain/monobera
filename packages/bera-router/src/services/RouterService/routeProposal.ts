...
import { type NewPath, SwapTypes, type SwapOptions, type SubgraphPoolBase, type PoolDictionary, PoolFilter, INFINITY, type PoolBase, type PoolPairBase, PoolTypes, type hopDictionary, type Swap } from "./types";
...
export function parseToPoolsDict(
    ...
    return Object.fromEntries(
        cloneDeep(pools)
            .filter(
                (pool) =>
                    pool.tokensList.length > 0 && pool.tokens[0]?.balance !== 0n
            )
    ...
    if (swapType === SwapTypes.SwapExactIn) {
        ...
        for (let i = 1; i < poolPairData.length; i++) {
            ...
            if (poolLimitExactIn <= limit) {
                ...
                limit = pushedLimit < poolLimitExactOut ? pushedLimit : poolLimitExactOut;
            }
        }
        if (limit === 0n) return 0n;
        return parseUnits(
    ...
    for (const hopToken in hopsIn) {
        if (hopsOut[hopToken]) {
            ...
            for (const poolInId of [...hopsIn[hopToken] ?? []]) {
                ...
                if (
                    normalizedLiquidity && normalizedLiquidity>= highestNormalizedLiquidityFirst
            ...
            for (const poolOutId of [...hopsOut[hopToken] ?? []]) {
                ...
                // Cannot be strictly greater otherwise highestNormalizedLiquidityPoolId = 0 if hopTokens[i] balance is 0 in this pool.
                if (
                    normalizedLiquidity && normalizedLiquidity >= highestNormalizedLiquiditySecond
                    )
                 {
...
export function createPath(tokens: string[], pools: PoolBase[] | undefined): NewPath {
