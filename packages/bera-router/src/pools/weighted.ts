...
import { type PoolPairBase, type PoolBase, PoolTypes, SwapTypes, type SubgraphPoolBase, type SubgraphToken } from "~/services/RouterService/types";
import { ONE, MathSol, _computeScalingFactor, _downscaleDown, _upscale } from "./basicOperations";
import { _calcBptOutGivenExactTokensIn, _calcTokenOutGivenExactBptIn, _calcOutGivenIn, _calcTokenInGivenExactBptOut, _calcBptInGivenExactTokensOut, _calcInGivenOut, _calcTokensOutGivenExactBptIn, _spotPriceAfterSwapExactTokenInForBPTOut, _spotPriceAfterSwapExactBPTInForTokenOut, _spotPriceAfterSwapExactTokenInForTokenOut, _spotPriceAfterSwapTokenInForExactBPTOut, _spotPriceAfterSwapBPTInForExactTokenOut, _spotPriceAfterSwapTokenInForExactTokenOut, _derivativeSpotPriceAfterSwapExactTokenInForBPTOut, _derivativeSpotPriceAfterSwapExactBPTInForTokenOut, _derivativeSpotPriceAfterSwapExactTokenInForTokenOut, _derivativeSpotPriceAfterSwapTokenInForExactTokenOut } from "./weightedMath";
...
export class WeightedPool implements PoolBase<WeightedPoolPairData> {
    ...
    parsePoolPairData(tokenIn: string, tokenOut: string): WeightedPoolPairData {
        ...
        const weightIn = parseUnits(`${Number(tI?.weight)}`, 18) * ONE / this.totalWeight;
        ...
        const weightOut = parseUnits(`${Number(tO?.weight)}`, 18) * ONE / this.totalWeight
    ...
    getNormalizedWeights(): bigint[] {
        return this.tokens.map((t) =>
            parseUnits(`${Number(t.weight)}`, 18) / this.totalWeight
        );
    }

    getNormalizedLiquidity(poolPairData: WeightedPoolPairData | undefined): bigint {
...
export const normaliseBalance = (
    ...
    return parseUnits(`${Number(token.balance)}`, 18) * parseUnits(`${Number(token.priceRate)}` ?? '1', 18)
};
...
export const denormaliseAmount = (
    amount: bigint,
    token: (Partial<Pick<SubgraphToken, 'priceRate'>> &
        Pick<SubgraphToken, 'decimals'>) | undefined
): bigint => {
    if(!token) return 0n
...
export const normaliseAmount = (
    ...
    const scalingFactor = _computeScalingFactor(BigInt(token.decimals));
    return _upscale(amount, scalingFactor) * parseUnits(`${Number(token.priceRate)}` ?? '1', 18)
};
