...
describe("SwapV3", () => {
  ...
  beforeAll(async () => {
    ...
    const fork = await startFork(ANVIL_NETWORKS.SEPOLIA, undefined, blockNo);
    rpcUrl = fork.rpcUrl;
    ...
    await approveSpenderOnTokens(client, testAddress, tokens, PERMIT2[chainId]);
  ...
  describe("query method should return correct updated", () => {
    describe("single swap", () => {
      ...
      test("GivenOut", async () => {
        ...
        expect(expected.expectedAmountIn.amount).to.eq(101002020040406n);
      });
    });
    describe("multi-hop swap", () => {
      describe("path with swaps only", () => {
        ...
        test("GivenOut", async () => {
          ...
          expect(expected.expectedAmountIn.amount).to.eq(593566581018333n);
        });
      });
      describe("path with exit", () => {
        ...
        test("GivenOut", async () => {
          ...
          expect(expected.expectedAmountIn.token).to.deep.eq(wethToken);
          expect(expected.expectedAmountIn.amount).to.eq(1591681955810971n);
        });
      });
    });
  });
    ...
    describe("multi-hop swap", () => {
      ...
      describe("query method should return correct updated", () => {
        test("GivenIn", async () => {
          ...
          expect(expected.swapKind).to.eq(SwapKind.GivenIn);
          expect(expected.pathAmounts).to.deep.eq([98985260225752491151n]);
          expect(expected.expectedAmountOut.token).to.deep.eq(daiToken);
        ...
        test("GivenOut", async () => {
          ...
          expect(expected.expectedAmountIn.token).to.deep.eq(usdcToken);
