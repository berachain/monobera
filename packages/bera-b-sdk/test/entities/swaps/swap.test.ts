    ...
    describe("path inputs show all start/end with same token", () => {
      ...
      describe("buffers", () => {
        ...
        test("should throw if buffers used for V2", () => {
          ...
          }).toThrowError("Unsupported swap: buffers not supported in V2.");
        });
  ...
  describe("buildCall max/min amounts", () => {
    describe("GivenIn", () => {
      test("18decimals>6decimals: minAmountOut to be 0.1% less then expected", () => {
        ...
        const tokenOut = new Token(
          1,
          pathTo6Decimals.tokens[pathTo6Decimals.tokens.length - 1].address,
          pathTo6Decimals.tokens[pathTo6Decimals.tokens.length - 1].decimals,
        );
        const expectedAmountOut = TokenAmount.fromHumanAmount(tokenOut, "1");
      ...
      test("6decimals>18decimals: minAmountOut to be 0.1% less then expected", () => {
        ...
        const tokenOut = new Token(
          1,
          pathFrom6Decimals.tokens[pathFrom6Decimals.tokens.length - 1].address,
          pathFrom6Decimals.tokens[pathFrom6Decimals.tokens.length - 1]
            .decimals,
        );
        const expectedAmountOut = TokenAmount.fromHumanAmount(tokenOut, "1");
    ...
    describe("GivenOut", () => {
      test("18decimals>6decimals: maxAmountIn to be 0.1% more then expected", () => {
        ...
        const expectedAmountIn = TokenAmount.fromHumanAmount(tokenIn, "1");
