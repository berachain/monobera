...
export function assertAddLiquidityUnbalanced(
  ...
  const expectedAmountsIn = poolState.tokens.map((t) => {
    const token = new Token(addLiquidityInput.chainId, t.address, t.decimals);
...
export function assertAddLiquiditySingleToken(
  ...
  const bptToken = new Token(addLiquidityInput.chainId, poolState.address, 18);
  ...
  if (wethIsEth) {
    expect(
      addLiquidityOutput.addLiquidityQueryOutput.amountsIn.some((t) =>
        t.token.isSameAddress(NATIVE_ASSETS[addLiquidityInput.chainId].wrapped),
      ),
    ).to.be.true;
  }
...
function assertTokenDeltas(
  ...
  if (wethIsEth) {
    const nativeAssetIndex = amountsWithoutBpt.findIndex((a) =>
      a.token.isSameAddress(NATIVE_ASSETS[addLiquidityInput.chainId].wrapped),
    );
