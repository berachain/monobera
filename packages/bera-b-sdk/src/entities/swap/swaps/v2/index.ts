...
export class SwapV2 implements SwapBase {
  public constructor({ chainId, paths, swapKind }: SwapInput) {
    ...
    this.assets = [
      ...new Set(this.paths.flatMap((p) => p.tokens).map((t) => t.address)),
    ];
  ...
  private async queryBatchSwap(
    ...
    if (this.swapKind === SwapKind.GivenIn) {
      return {
        ...
        expectedAmountOut: TokenAmount.fromRawAmount(
          this.outputAmount.token,
          abs(result[this.assets.indexOf(this.outputAmount.token.address)]),
        ),
        amountIn: this.inputAmount,
      };
    }
    return {
      ...
      expectedAmountIn: TokenAmount.fromRawAmount(
        this.inputAmount.token,
        abs(result[this.assets.indexOf(this.inputAmount.token.address)]),
      ),
      amountOut: this.outputAmount,
    };
  }
  ...
  private value(limitAmount: TokenAmount, wethIsEth: boolean): bigint {
    ...
    if (
      wethIsEth &&
      this.inputAmount.token.address === NATIVE_ASSETS[this.chainId].wrapped
    ) {
      if (this.swapKind === SwapKind.GivenIn) value = this.inputAmount.amount;
  ...
  private callDataSingleSwap(
    ...
    if (wethIsEth) {
      swap.assetIn = this.convertWrappedToZero(this.chainId, swap.assetIn);
      swap.assetOut = this.convertWrappedToZero(this.chainId, swap.assetOut);
    }
  ...
  private callDataBatchSwap(
    ...
    return encodeFunctionData({
      ...
      args: [
        ...
        this.swaps as BatchSwapStep[],
        wethIsEth
          ? this.assets.map((a) => this.convertWrappedToZero(this.chainId, a))
          : this.assets,
        funds,
        limits,
        deadline,
