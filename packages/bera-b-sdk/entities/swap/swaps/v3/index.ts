...
export class SwapV3 implements SwapBase {
  ...
  private async queryBatchSwap(
    ...
    if (this.swapKind === SwapKind.GivenIn) {
      ...
      if (result[1].length !== 1)
        throw Error("Swaps only support paths with matching tokenIn>tokenOut");
  ...
  private callDataBatchSwap(
    ...
    if (this.swapKind === SwapKind.GivenIn) {
      if ((this.swaps as SwapPathExactAmountIn[]).length !== pathLimits.length)
      ...
      if ((this.swaps as SwapPathExactAmountOut[]).length !== pathLimits.length)
        throw Error("Must have a limit for each path.");
