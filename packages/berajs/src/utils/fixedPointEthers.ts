...
export const MAX_PRICE = BigNumber.from(
...
export function toQ64(val: number): BigNumber {
  const multFixed = Math.round(val * PRECISION);
...
export function toFixedGrowth(mult: number) {
  const multFixed = Math.round(mult * PRECISION);
  return BigNumber.from(multFixed).mul(Q_48).div(PRECISION);
}
...
export function toSqrtPrice(price: number): bigint {
  const sqrtFixed = Math.round(Math.sqrt(price) * PRECISION);
  const bigNumberSqrt = BigNumber.from(sqrtFixed).mul(Q_64).div(PRECISION);
  return bigNumberSqrt.toBigInt();
}

export function fromSqrtPrice(val: BigNumber) {
  const root = val.mul(PRECISION).div(Q_64).toNumber() / PRECISION;
  return root * root;
}
...
export function encodeCrocPrice(price: number): bigint {
  ...
  return bnSeed.mul(BigNumber.from(2).pow(scale)).toBigInt();
}
